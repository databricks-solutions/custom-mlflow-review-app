...
class SchemaGenerator:
    ...
    def generate_schemas_for_issues(
        ...
        # Add baseline quality schemas if needed
        if len(schemas) < 3:
    ...
    def _create_feedback_schema(self, issue: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if schema_type == 'numerical':
            schema.update({
                'min': 1,
                'max': 5,
                'evaluation_guidance': self._generate_rating_guidance(issue)
            })
    ...
    def _create_baseline_schemas(self, agent_understanding: str) -> List[Dict[str, Any]]:
        """Create baseline quality schemas as fallback."""
        ...
        return [
            {
                ...
                'rationale': 'Baseline quality assessment',
                'priority_score': 0,
                'grounded_in_traces': [],
                'all_affected_traces': [],
            ...
            },
            {
                ...
                'label_type': 'FEEDBACK',
                'schema_type': 'text',
                'description': 'Provide any additional feedback or observations',
                'max_length': 500,
                'rationale': 'Capture any issues not covered by other schemas',
                'priority_score': 0,
                'grounded_in_traces': [],
                'all_affected_traces': [],
                'affected_trace_count': 0
