...
class ExperimentAnalyzer:
  ...
  async def analyze_experiment(
    self, experiment_id: str, focus: str = 'comprehensive', trace_sample_size: int = 50
  ) -> Dict[str, Any]:
    ...
    try:
      ...
      discovery_result = await self.issue_discovery.discover_issues(
        experiment_data['traces'], experiment_data['experiment_info']
      )
      ...
      schemas = self.schema_generator.generate_schemas_for_issues(
        discovery_result['issues'], discovery_result['agent_understanding']
      )

      # Step 4: Generate comprehensive report
      self.logger.info('Step 4: Generating analysis report...')
      report = self.report_generator.generate_report(experiment_data, discovery_result, schemas)
      ...
      structured_analysis = self._structure_analysis(
        experiment_data, discovery_result, schemas, report
      )

      # Step 6: Store to MLflow artifacts
      self.logger.info('Step 6: Storing analysis to MLflow...')
      storage_result = await self._store_analysis(experiment_id, structured_analysis)
  ...
  async def _gather_experiment_data(self, experiment_id: str, sample_size: int) -> Dict[str, Any]:
    ...
    # Get all traces (up to sample_size) - add debugging
    self.logger.info(f'Searching for traces in experiment {experiment_id}')
    raw_traces = search_traces(
      experiment_ids=[experiment_id], max_results=sample_size, order_by=['timestamp_ms DESC']
    )

    self.logger.info(f'Found {len(raw_traces)} raw traces from search_traces')

    # If no traces found, log warning and return early with diagnostic info
    if not raw_traces:
      self.logger.warning(f'No traces found in experiment {experiment_id}. This could indicate:')
      self.logger.warning('1. Experiment has no traces')
      self.logger.warning('2. Insufficient permissions to read traces')
      self.logger.warning('3. Invalid experiment ID')

      return {
        'experiment_info': {
          'experiment_id': experiment_info.get('experiment', {}).get(
            'experiment_id', experiment_id
          ),
          'name': experiment_info.get('experiment', {}).get('name', 'Unknown'),
          'artifact_location': experiment_info.get('experiment', {}).get('artifact_location'),
          'lifecycle_stage': experiment_info.get('experiment', {}).get('lifecycle_stage'),
          'creation_time': experiment_info.get('experiment', {}).get('creation_time'),
          'last_update_time': experiment_info.get('experiment', {}).get('last_update_time'),
          'tags': experiment_info.get('experiment', {}).get('tags', {}),
        },
        'traces': [],
        'total_traces': 0,
        'sample_size_requested': sample_size,
        'diagnostic_issue': 'no_traces_found',
      }
        ...
        if trace.data and trace.data.spans:
          for span in trace.data.spans:
            span_dict = {
              ...
              'start_time_ms': getattr(span, 'start_time_ns', 0) // 1000000
              if hasattr(span, 'start_time_ns')
              else getattr(span, 'start_time_ms', 0),
              'end_time_ms': getattr(span, 'end_time_ns', 0) // 1000000
              if hasattr(span, 'end_time_ns')
              else getattr(span, 'end_time_ms', 0),
    ...
    self.logger.info(f'Successfully processed {len(traces)} traces for analysis')

    if len(traces) == 0:
      self.logger.warning(
        'After processing, 0 traces remain - all traces may have had processing errors'
      )

    # Extract experiment data from the nested structure
    exp_data = experiment_info.get('experiment', experiment_info)
    ...
    return {
      'experiment_info': {
        ...
        'creation_time': exp_data.get('creation_time'),
        'last_update_time': exp_data.get('last_update_time'),
  ...
  def _structure_analysis(
    ...
    return {
      ...
      'raw_sme_analysis': {
        ...
        'schema_recommendations': {'recommended_schemas': schemas_with_traces},
      },
      'detected_issues': issues_with_full_traces,  # For UI
      'schemas_with_label_types': schemas_with_traces,  # For UI
    }

  async def _store_analysis(self, experiment_id: str, analysis: Dict[str, Any]) -> Dict[str, str]:
