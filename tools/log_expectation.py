#!/usr/bin/env python3
"""Tool for logging expectations on MLflow traces.
...
Usage:
    uv run python tools/log_expectation.py <trace_id> <expectation_key> <expectation_value> [--comment "Optional comment"]
...
Examples:
    ...
    uv run python tools/log_expectation.py tr-123 max_response_time 5.0 --comment "Response should be under 5 seconds"
...
def log_trace_expectation(trace_id: str, expectation_key: str, expectation_value: Any, expectation_comment: str = None) -> dict:
...
def main():
    ...
    parser.add_argument('expectation_key', help='Expectation key/name (e.g., "response_format", "required_fields")')
    parser.add_argument('expectation_value', help='Expectation value (string, number, boolean, JSON array, or JSON object)')
