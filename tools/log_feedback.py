#!/usr/bin/env python3
"""Tool for logging feedback on MLflow traces.
...
Usage:
    uv run python tools/log_feedback.py <trace_id> <feedback_key> <feedback_value> [--comment "Optional comment"]
...
Examples:
    ...
    uv run python tools/log_feedback.py tr-123 category "excellent" --comment "Very thorough response"
...
def log_trace_feedback(trace_id: str, feedback_key: str, feedback_value: Any, feedback_comment: str = None) -> dict:
...
def main():
    ...
    parser.add_argument('feedback_value', help='Feedback value (string, number, boolean, or JSON array)')
