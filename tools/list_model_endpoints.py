        ...
        if format_output == 'json':
            ...
            for endpoint in endpoints:
                ...
                endpoint_data.append({
                    'name': endpoint.name,
                    'state': state,
                    'creator': endpoint.creator or 'N/A'
                })
            ...
            for endpoint in endpoints:
                ...
                if endpoint.state and hasattr(endpoint.state, 'ready'):
                    ...
                    state = 'UNKNOWN'
                creator = endpoint.creator[:29] if endpoint.creator and len(endpoint.creator) > 29 else (endpoint.creator or 'N/A')
                last_updated = str(endpoint.last_updated_timestamp) if endpoint.last_updated_timestamp else 'N/A'
