...
def test_cleanup():
  ...
  response = requests.post(
    f'{BASE_URL}/api/review-apps/{review_app_id}/schemas', json=test_schema, headers=headers
  )
  ...
  if response.status_code == 200:
    ...
    if test_schema['name'] in schema_names:
      print('✅ Schema confirmed in list')
    else:
      print('❌ Schema not found in list')
      return False

  # Delete the schema
  print('\n4. Deleting test schema...')
  response = requests.delete(
    f"{BASE_URL}/api/review-apps/{review_app_id}/schemas/{test_schema['name']}", headers=headers
  )
  ...
  if response.status_code == 200:
    ...
    if test_schema['name'] not in schema_names:
      print('✅ Schema confirmed deleted from list')
    else:
      print('❌ Schema still exists in list')
      return False
  ...
  if response.status_code == 404:
    print('✅ Direct schema lookup returns 404 (correctly deleted)')
  else:
