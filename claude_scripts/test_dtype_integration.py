...
class DTypeIntegrationTest:
    ...
    def setup(self) -> bool:
        ...
        if isinstance(trace, dict) and "info" in trace:
            ...
            print(f"❌ Unexpected trace structure: {trace.keys() if isinstance(trace, dict) else type(trace)}")
            return False
    ...
    def test_boolean_dtype(self) -> bool:
        ...
        true_assessment = next(
            (a for a in assessments if a.get("name") == f"{test_name}_true"),
            None
        )
        
        if not true_assessment:
            print(f"  ❌ True assessment not found in metadata")
            return False
        ...
        false_assessment = next(
            (a for a in assessments if a.get("name") == f"{test_name}_false"),
            None
        )
        
        if not false_assessment:
            print(f"  ❌ False assessment not found in metadata")
            return False
    ...
    def test_numeric_dtypes(self) -> bool:
        ...
        for test_name, expected_value, expected_type in test_cases:
            assessment = next(
                (a for a in assessments if a.get("name") == f"test_dtype_numeric_{test_name}"),
                None
            )
    ...
    def test_string_dtype(self) -> bool:
        ...
        for test_name, expected_value in test_cases:
            assessment = next(
                (a for a in assessments if a.get("name") == f"test_dtype_string_{test_name}"),
                None
            )
    ...
    def test_categorical_schema_with_booleans(self) -> bool:
        ...
        # Check "True" option
        true_option = next(
            (a for a in assessments if a.get("name") == f"{schema_name}_true_option"),
            None
        )
        
        if not true_option:
            print(f"  ❌ 'True' option assessment not found")
            return False
            
        if true_option.get("value") != "True" or not isinstance(true_option.get("value"), str):
            print(f"  ❌ 'True' option incorrect: {true_option.get('value')} (type: {type(true_option.get('value')).__name__})")
            return False
        print(f"  ✅ 'True' option stored as string")
        
        # Check "False" option
        false_option = next(
            (a for a in assessments if a.get("name") == f"{schema_name}_false_option"),
            None
        )
        
        if not false_option:
            print(f"  ❌ 'False' option assessment not found")
            return False
            
        if false_option.get("value") != "False" or not isinstance(false_option.get("value"), str):
            print(f"  ❌ 'False' option incorrect: {false_option.get('value')} (type: {type(false_option.get('value')).__name__})")
            return False
        print(f"  ✅ 'False' option stored as string")
    ...
    def test_array_dtype(self) -> bool:
        ...
        for test_name, expected_value in test_cases:
            assessment = next(
                (a for a in assessments if a.get("name") == f"test_dtype_array_{test_name}"),
                None
            )
    ...
    def test_dict_dtype(self) -> bool:
        ...
        for test_name, expected_value in test_cases:
            assessment = next(
                (a for a in assessments if a.get("name") == f"test_dtype_dict_{test_name}"),
                None
            )
    ...
    def test_update_preserves_dtype(self) -> bool:
        ...
        assessments = response.json().get("info", {}).get("assessments", [])
        assessment = next(
            (a for a in assessments if a.get("name") == "test_dtype_update"),
            None
        )
        
        if not assessment:
            print(f"  ❌ Updated assessment not found")
            return False
            
        if assessment.get("value") is not False or not isinstance(assessment.get("value"), bool):
            print(f"  ❌ Boolean type not preserved: {assessment.get('value')} (type: {type(assessment.get('value')).__name__})")
            return False
        print(f"  ✅ Boolean type preserved after update")
        ...
        response = requests.patch(
            ...
            json={
                ...
                "assessment": {
                    "value": 2.718,
                    "rationale": "Updated numeric value"
                }
            }
        )
        ...
        assessments = response.json().get("info", {}).get("assessments", [])
        assessment = next(
            (a for a in assessments if a.get("name") == "test_dtype_update_numeric"),
            None
        )
        
        if not assessment:
            print(f"  ❌ Updated numeric assessment not found")
            return False
        ...
        if not isinstance(value, (int, float)) or abs(value - 2.718) > 0.001:
            ...
            return False
        print(f"  ✅ Numeric type preserved after update")
    ...
    def test_null_and_edge_cases(self) -> bool:
        ...
        response = requests.post(
            ...
            json={
                "assessment": {
                    "name": "test_dtype_null",
                    "value": None,
                    "rationale": "Testing null value"
                }
            }
        )
        ...
        large_assessment = next(
            (a for a in assessments if a.get("name") == "test_dtype_large_number"),
            None
        )
        ...
        small_assessment = next(
            (a for a in assessments if a.get("name") == "test_dtype_small_decimal"),
            None
        )
        ...
        for schema_name in self.created_schemas:
            try:
                response = requests.delete(
                    f"{BASE_URL}/api/label-schemas/{schema_name}",
                    headers=self.get_headers()
                )
