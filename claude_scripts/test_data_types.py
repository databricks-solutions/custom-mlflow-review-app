...
def test_assessment_data_types():
    ...
    if isinstance(traces[0], dict) and "info" in traces[0]:
        ...
        print(f"   ❌ Unexpected trace structure: {traces[0].keys() if isinstance(traces[0], dict) else type(traces[0])}")
        return False
    ...
    bool_assessment = next(
        (a for a in assessments if a.get("name") == "test_boolean"),
        None
    )
    ...
    if bool_assessment:
        ...
        if isinstance(value, bool):
            print(f"   ✅ Boolean assessment stored correctly")
            print(f"      Value: {value} (type: {value_type})")
        else:
            print(f"   ❌ Boolean assessment stored as wrong type!")
            print(f"      Value: {value} (type: {value_type})")
    ...
    num_assessment = next(
        (a for a in assessments if a.get("name") == "test_numeric"),
        None
    )
    ...
    if num_assessment:
        ...
        if isinstance(value, (int, float)):
            print(f"   ✅ Numeric assessment stored correctly")
            print(f"      Value: {value} (type: {value_type})")
        else:
            print(f"   ❌ Numeric assessment stored as wrong type!")
            print(f"      Value: {value} (type: {value_type})")
    ...
    return (
        bool_assessment and isinstance(bool_assessment.get("value"), bool) and
        num_assessment and isinstance(num_assessment.get("value"), (int, float))
    )
