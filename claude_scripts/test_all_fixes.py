...
def test_pass_fail_schema_creation():
  ...
  response = requests.post(f'{BASE_URL}/api/label-schemas', json=schema_data, headers=get_headers())
  ...
  if result.get('categorical', {}).get('options') == ['True', 'False']:
    print('   ✅ Pass/Fail schema created with correct True/False options')
    return True
  else:
...
def test_boolean_value_storage():
  ...
  if isinstance(traces[0], dict) and 'info' in traces[0]:
    ...
    print('   ❌ Unexpected trace structure')
    return False

  # Test with True value
  bool_feedback = {
    'assessment': {'name': 'test_boolean_true', 'value': True, 'rationale': 'Testing True boolean'}
  }
  ...
  true_assessment = next((a for a in assessments if a.get('name') == 'test_boolean_true'), None)

  # Check False assessment
  false_assessment = next((a for a in assessments if a.get('name') == 'test_boolean_false'), None)
...
def test_schema_type_detection():
  ...
  test_schema = next((s for s in schemas if s.get('name') == 'test_pass_fail_fix'), None)

  if test_schema:
    options = test_schema.get('categorical', {}).get('options', [])
    if options == ['True', 'False']:
      print('   ✅ Pass/Fail schema has correct True/False options')
      return True
    else:
...
def cleanup_test_schemas():
  ...
  try:
    ...
    response = requests.delete(
      f'{BASE_URL}/api/label-schemas/test_pass_fail_fix', headers=get_headers()
    )
