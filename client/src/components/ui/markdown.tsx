    ...
    <div className={cn(
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          h1: ({ children, ...props }) => (
            <h1 className="scroll-m-20 text-3xl font-bold tracking-tight lg:text-4xl border-b border-border pb-2 mb-4 text-foreground" {...props}>
              {children}
            </h1>
          ),
          h2: ({ children, ...props }) => (
            <h2 className="scroll-m-20 text-2xl font-semibold tracking-tight lg:text-3xl mt-8 mb-4 text-foreground" {...props}>
              {children}
            </h2>
          ),
          h3: ({ children, ...props }) => (
            <h3 className="scroll-m-20 text-xl font-semibold tracking-tight lg:text-2xl mt-6 mb-3 text-foreground" {...props}>
              {children}
            </h3>
          ),
          h4: ({ children, ...props }) => (
            <h4 className="scroll-m-20 text-lg font-semibold tracking-tight mt-4 mb-2 text-foreground" {...props}>
              {children}
            </h4>
          ),
          // Enhanced code blocks
          pre: ({ children, ...props }) => (
            <pre className="overflow-x-auto rounded-lg border bg-card p-4 text-sm shadow-sm my-6 text-foreground" {...props}>
              {children}
            </pre>
          ),
          code: ({ className, children, ...props }) => {
            ...
            if (isInline) {
              return (
                <code className="relative rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm font-medium text-foreground" {...props}>
                  {children}
                </code>
              );
            }
          ...
          blockquote: ({ children, ...props }) => (
            <blockquote className="mt-6 border-l-4 border-primary pl-6 py-3 bg-muted/50 rounded-r-md italic text-muted-foreground" {...props}>
              {children}
            </blockquote>
          ),
          // Enhanced tables
          table: ({ children, ...props }) => (
            <div className="my-6 w-full overflow-y-auto">
              <table className="w-full border-collapse overflow-hidden rounded-md border shadow-sm" {...props}>
                {children}
              </table>
            </div>
          ),
          ...
          th: ({ children, ...props }) => (
            <th className="border border-border bg-muted p-3 text-left font-semibold text-foreground" {...props}>
              {children}
            </th>
          ),
          td: ({ children, ...props }) => (
