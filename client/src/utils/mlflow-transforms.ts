    ...
    inputs: (() => {
      try {
        return attributes['mlflow.spanInputs'] ? 
          JSON.parse(attributes['mlflow.spanInputs'] as string) : undefined;
      } catch {
        return attributes['mlflow.spanInputs'];
      }
    })(),
    outputs: (() => {
      try {
        return attributes['mlflow.spanOutputs'] ? 
          JSON.parse(attributes['mlflow.spanOutputs'] as string) : undefined;
      } catch {
        return attributes['mlflow.spanOutputs'];
      }
    })(),
    ...
    return {
      info: {
        ...
        execution_duration: trace.info.execution_duration_ms ? 
          `${trace.info.execution_duration_ms}ms` : undefined,
      },
      data: {
